...
public class PetController {
    ...
    @PostMapping
    public ResponseEntity<PetDTO> createPet(@Validated @RequestBody PetDTO petDTO) {
        PetDTO createdPet = petService.createPet(petDTO);
        return new ResponseEntity<>(createdPet, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<PetDTO> updatePet(@PathVariable("id") Integer id, @Validated @RequestBody PetDTO petDTO) {
